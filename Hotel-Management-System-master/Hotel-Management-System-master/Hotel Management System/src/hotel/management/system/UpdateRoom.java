	...
	public UpdateRoom() throws SQLException {
		...
		b1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					conn c = new conn();
					ResultSet rs1 = c.s.executeQuery("SELECT room_number FROM customer WHERE id_number = '" + id + "'");
					...
					ResultSet rs2 = c.s.executeQuery(
							"SELECT available, status FROM room WHERE room = '" + txt_Room.getText() + "'");
					if (rs2.next()) {
		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					conn c = new conn();
					String str = "UPDATE room SET available = '" + txt_Ava.getText() + "', status = '"
							+ txt_Status.getText() + "' WHERE room = '" + txt_Room.getText() + "'";
